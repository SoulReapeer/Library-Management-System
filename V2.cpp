...
// List all currently borrowed books with member info
void listBorrowedBooks() {
    ifstream borrowFile("borrow.dat", ios::binary);
    ifstream bookFile("books.dat", ios::binary);
    ...
    cout << "\n--- Borrowed Books ---\n";
    cout << left << setw(5) << "BID" << setw(30) << "Title" << setw(20) << "Author"
         << setw(5) << "MID" << setw(20) << "Member" << setw(12) << "Borrow Date" << endl;

    while(borrowFile.read(reinterpret_cast<char*>(&br), sizeof(Borrow))) {
        if(br.returnDate == "") {
            // Find book info
            bookFile.clear(); bookFile.seekg(0);
            while(bookFile.read(reinterpret_cast<char*>(&b), sizeof(Book))) {
                if(b.id == br.bookId) break;
            }
            // Find member info
            memberFile.clear(); memberFile.seekg(0);
            while(memberFile.read(reinterpret_cast<char*>(&m), sizeof(Member))) {
                if(m.id == br.memberId) break;
            }
            cout << setw(5) << b.id << setw(30) << b.title << setw(20) << b.author
                 << setw(5) << m.id << setw(20) << m.name << setw(12) << br.borrowDate << endl;
        }
    }
